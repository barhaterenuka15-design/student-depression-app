...
# ---------------------------------------------------------------------
# Load model + the exact training column list once, at startup.
# The model is stored as a compressed joblib file (model.joblib) rather
# than a raw pickle -- same model, same predictions, but ~6x smaller on
# disk (76MB -> 12MB), which keeps it well under GitHub/Render's file
# size limits and makes deploys faster.
#
# model_columns.pkl is required because pd.get_dummies() only creates
